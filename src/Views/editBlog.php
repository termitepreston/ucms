...
<form id="blogEntryForm" action="" method="post">
    ...
    <div class="form-group">
        ...
        <input type="text" id="blogEntryTitle" name="blogEntryTitle" onblur="validateBlogEntryTitle()" onfocus="clearError('blogEntryTitleError')" value="<?= $blog["title"]  ?? '' ?>" required>
        <div id="blogEntryTitleError" class="error" style="display: none;"></div>
    </div>

    <div class="form-group">
        <label for="blogEntrySlug">Blog Entry Slug:</label>
        <input type="text" id="blogEntrySlug" name="blogEntrySlug" onblur="validateBlogEntrySlug()" onfocus="clearError('blogEntrySlugError')" value="<?= $blog["slug"]  ?? '' ?>" required>
        <div id="blogEntrySlugError" class="error" style="display: none;"></div>
    </div>

    <div class="form-group">
        <label for="blogEntryMarkdown">Markdown Text:</label>
        <textarea id="blogEntryMarkdown" name="blogEntryMarkdown" rows="10" required>
                <?= $blog["content"] ?? '' ?>
            </textarea>
    </div>
