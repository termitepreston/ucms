...
class UserController
{
    ...
    public function register()
    {
        ...
        if ($isSubmitted) {
            $results = $this->validateUserRegistrationForm();

            $isValid = empty($results['errors']);

            if ($isValid) {

                if ($this->registerRegularUser($results['sanitizedInput'])) {
                    $this->loginInternal($results['sanitizedInput']['username'], $results['sanitizedInput']['password']);
        ...
        require_once self::VIEWS . 'register.php';
     ...
     * @return array<string, array<string, string>> An associative array of validation errors. Empty array if no errors.
     */
    private function validateUserRegistrationForm(): array
    {
        $errors = [];
        $sanitizedInput = [];
    ...
    private function registerRegularUser(array $input): bool
    {
        $username = $input['username'];
        $firstName = $input['firstName'];
        $lastName = $input['lastName'];
        ...
        try {
            $stmt = $this->pdo->prepare(
                "INSERT INTO users 
                (username, first_name, last_name, email, profile_picture_path, bio, password_hash)
            VALUES
                (:username, :firstName, :lastName, :email, :profilePhoto, :bio, :passwordHash)"
            );
            ...
            $stmtRBAC->bindValue(':userId', $userId);
            $stmtRBAC->bindValue(':roleName', 'user');
